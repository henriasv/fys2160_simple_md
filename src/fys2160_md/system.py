...
class System:
    """Mutable periodic system: geometry, velocities and evolving MD state.

    Use FCC or Random for generated geometries; use System for explicit arrays.
    N counts atoms for atomic systems and molecules for diatomic systems.
    Advance this object with Simulation.run(system, ...).
    """

    def __init__(self, positions, box, *, velocities=None, masses=None,
                 model='atomic', temperature=2., ensemble='nvt', timestep=None,
        ...
        self.particles = _integer(len(x) if model=='atomic' else len(x)//2, 'N', 2)
        ...
        rng=np.random.default_rng(self.seed) if _initial_rng is None else _initial_rng
        if velocities is None:
            self._v=np.ascontiguousarray(rng.normal(size=x.shape)/np.sqrt(self._mass[:,None]))
            self._v-=(self._v*self._mass[:,None]).sum(axis=0)/self._mass.sum()
            if model=='diatomic-rigid':self._project_velocities()
            kinetic_twice=np.sum(self._mass[:,None]*self._v**2)
            self._v*=np.sqrt(self.dof*self._settings['temperature']/kinetic_twice)
        else:
            self._v=np.array(velocities,dtype=float,order='C',copy=True)
            if self._v.shape!=x.shape or not np.isfinite(self._v).all():
            ...
            if np.linalg.norm(momentum.sum(axis=0))>1e-10*max(1.,np.linalg.norm(momentum)):
                raise ValueError('Remove centre-of-mass velocity before using explicit velocities.')
        if model=='diatomic-rigid':
            bond=self._bond_vectors()
            ...
            if np.max(np.abs((bond*(self._v[::2]-self._v[1::2])).sum(axis=1)))>1e-10:
    ...
    def from_arrays(cls, positions, velocities, box, *, masses=None, model='atomic', **kwargs):
        ...
        """
        return cls(positions,box,velocities=velocities,masses=masses,model=model,**kwargs)
    ...
    @property
    def rho(self):
        return self.particles/self.box.volume
    ...
    def from_run(cls, run, *, output_dir=None):
        ...
        with np.load(run.path/'checkpoint.npz', allow_pickle=False) as data:
            obj=cls(data['positions'],data['box'],velocities=data['velocities'],masses=data['masses'],**c)
    ...
    def _run(self, timestep=None, steps=1000, *, ensemble=None, temperature=None,
    ...
    def __repr__(self):
        return f'System(model={self.model!r}, particles={self.particles}, step={self.step}, settings={self.settings})'
